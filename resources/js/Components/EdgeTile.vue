<template>
    <div :id="props.config.property_id" class="grid grid-rows-4 grid-cols-1 text-[12px] text-center">
        <div :class="props.color + (props.color ? ' border-b-2 border-black' : '')"></div>
        <div class="p-2">{{ props.config.name }}</div>
        <div class="p-2 text-lg">{{ props.config.property_id == props.position ? '*' : '' }}</div>
